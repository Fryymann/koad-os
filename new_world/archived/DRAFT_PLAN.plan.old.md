...
- [ ]  **Spine retirement doc** — Write a formal deprecation note mapping every Spine function to its new home (Citadel, CASS, or agent-local).
...
- [ ]  **🔴 Dark Mode local persistence & Reconciliation** — When an agent loses Citadel connectivity, they must write to a **standardized `.md` or `.json` structure** that CASS can reliably parse during Brain Drain or reconnect reconciliation. Define: path convention (`<project_dir>/.koad-dark/<agent>/<session_id>.md`?), metadata format (TOML/JSON frontmatter), and CASS parser contract. **Sovereignty Hierarchy:** Implement the `Dood > Citadel State > Local Save` reconciliation hierarchy. Conflicting local saves must be moved to `.koad-conflict/` for manual review.
...
- [ ]  **🟡 EndOfWatch schema standardization** — For TCH (Temporal Context Hydration) to work, EndOfWatch summaries need a **strictly enforced schema** — not free-form text. 
    - **Format:** Standardized JSON/TOML frontmatter block followed by Markdown body. 
    - **Required Fields:** `agent`, `session_id`, `task_id`, `status` (COMPLETED|STALLED|BLOCKED), `trace_id`, `timestamp`, `project`, `worked_on[]`, `decisions[]`, `blockers[]`, `next_steps[]`, `learnings[]`. *(Addresses Tyr's "EndOfWatch schema rigidity" gap.)*
...
5. **Gate 1.5 (Proto Validation):** Verify `citadel.proto` supports bidirectional streaming for logs/signals before building the service.
...
5. **Gate 2.5 (Zero-Trust Audit):** Run a "Red Team" test: can a Tier 2 agent access another agent's Personal Bay filesystem map or sovereign Redis keys?
...
3. **Dark Mode persistence & Reconciliation** — Implement standardized local save format (structured `.md` with TOML frontmatter) and CASS parser. Implement the **Sovereignty Hierarchy** (`Dood > Citadel State > Local Save`) for reconnect reconciliation, ensuring conflicting local saves are quarantined to `.koad-conflict/`. *(Addresses Tyr's "Dark Mode reconciliation" gap.)*
...
6. **Gate 3.5 (Migration Dry Run):** Validate `koad system migrate-v5` on a subset of legacy data before the final Spine archive.
...
```
DORMANT → DOCKING → HYDRATING → ACTIVE → WORKING → DARK → STASIS → TEARDOWN → DORMANT
```
...
| DARK → STASIS | Timeout (>5m in DARK state) | **"Warm Sleep" Protocol:** L1 working memory (Redis) is preserved for 5 minutes before full teardown. Allows instant resume after minor jitter. |
| STASIS → WORKING | Heartbeat Restored (within 5m) | Instant resume from preserved L1 state. |
| STASIS → TEARDOWN | Timeout (>5m in STASIS) | Citadel initiates teardown; agent session is considered lost |
...
**DARK state recovery:** When an agent enters DARK state, it continues working locally using Agent-Level Cognition. On reconnect (DARK → WORKING), CASS performs a reconciliation diff: local `.md` saves are merged with bay state, and any conflicting writes surface as `MEMORY_CONFLICT` events. *(Addresses Tyr's "Personal Bay warm sleep" gap.)*
...
- The Personal Bay does not hold *actual* tokens in its long-term state; it holds **references**. 
- The Citadel retrieves the token from the Secret Manager only when requested by a validated agent session and serves it with a TTL. *(Addresses Tyr's "Credential JIT delivery" gap.)*
...
**Technical Enforcement (Jailing):**
- **Sanctuary Rule enforced** at the worktree level. `koad-agent` sets `KOAD_WORKSPACE_ROOT` during boot. 
- The Citadel validates all file-operation tool calls (via MCP or wrapper) against this root. 
- For higher-tier enforcement, use **Bubblewrap (bwrap)** or `chroot` to jail the agent's "Body" shell to the task-specific worktree and its required support directories. *(Addresses Tyr's "Workspace Manager jailing" gap.)*
...
- **Spam Protection (Quotas & Rate Limits):**
    - **Priority Quota:** Each agent is limited to **3 HIGH-priority signals per hour**. 
    - **Global Rate Limit:** NORMAL and LOW signals are rate-limited per sender (e.g., 20 per hour) to prevent accidental infinite loops or floods. *(Addresses Tyr's "A2A-S spam protection" gap.)*
...
2. **Packetizes** events into "Signal Packets" — batched, compressed, high-signal summaries
3. **Broadcasts** packets to Redis Streams for multi-consumer consumption

**Dynamic Frequency Scaling (Pressure Management):**
- **Adaptive Batching:** The Signal Corps should batch based on *volume* or *time*, whichever hits a threshold first (e.g., 500ms or 50KB). 
- Under high-volume logging (e.g., verbose builds), the frequency increases to prevent buffer lag, but never faster than 10Hz to protect Redis throughput. *(Addresses Tyr's "Signal Corps pressure" gap.)*
