...
**Station**
Level 2 in the Workspace Hierarchy. A project hub directory containing multiple related repositories (e.g., `~/skylinks/`). Defined by a `koad.toml` with a `[station]` section. Context hydration at Station level includes cross-repo summaries. See: [Workspace Hierarchy](./architecture/workspace-hierarchy.md).
