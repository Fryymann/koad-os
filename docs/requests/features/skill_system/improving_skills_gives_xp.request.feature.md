...
Every measurable improvement in a skill's eval score earns the implementing agent **Experience Points (XP)**. XP is not cosmetic — it functions as a signal layer for agent capability reputation within KoadOS.

**Award triggers and values (suggested defaults):**

| **Event** | **XP Award** | **Notes** |
| --- | --- | --- |
| First passing eval (initial deploy at ≥ 80%) | +50 XP | Awarded once per skill, on first promotion. |
| Score improvement ≥ 5% from prior run | +20 XP | Incremental gains count. Compounding improvement rewarded. |
| Score improvement ≥ 15% from prior run | +50 XP | Replaces the +20 award — not additive. |
| Zero-regression revision (all prior passing scenarios still pass) | +10 XP bonus | Added on top of score improvement award if applicable. |
| Skill reaches ≥ 95% eval score | +100 XP (one-time) | Excellence threshold bonus. Awarded once per skill. |
| Skill rollback (regression below deploy threshold) | −15 XP | Penalty for deploying a skill that regresses in production. |

**XP is logged in `_eval/improvement-log.md`** alongside the version bump and score delta. Format:

```
1.0.0 → 1.0.1: score 74% → 83% (+9%), zero regressions | +20 XP +10 XP bonus = +30 XP | agent: tyr
```

**Agent XP ledger:** Each agent maintains a cumulative XP total across all skills they author or improve. The ledger lives in the agent's CASS L1 profile record. XP is queryable via KoadStream and surfaced in agent status reports.

**Design note for Tyr:** XP values above are starting defaults. The grading schema's `eval_pass_threshold` and XP award thresholds should be tunable per-skill in `SKILL.md` frontmatter if needed for specialized skills with different difficulty baselines.
...
7. **Promote** — If overall score ≥ `eval_pass_threshold` and no regressions, the skill is deployed. Version is tagged in `improvement-log.md`. **XP awards are calculated and written to the log at this step.**
