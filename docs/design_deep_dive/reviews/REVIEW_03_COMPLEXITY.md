...
**The Vulnerability:** KoadOS executes tools as the `ideans` user. Even if we inject a sandbox `.env`, Sky can simply execute `run_shell_command("cat ~/.config/gcloud/credentials.db")` or `cat ~/.koad-os/koad.json` and extract the production API keys. The "Isolation Mandate" is currently security theater.
