...
def test_developer_sandbox_denial(spine, redis_client):
    ...
    # 1. Elevate to Developer role in config/kernel.toml
    config/kernel.toml = spine.koad_home / "config/kernel.toml"
    config = json.loads(config/kernel.toml.read_text())
    config["identity"]["role"] = "Developer"
    config/kernel.toml.write_text(json.dumps(config))
