...
- Respect the Sanctuary Rule — do not access ~/.koad-os/ or config/.
```
...
- Respect the Sanctuary Rule: Never edit ~/.koad-os/ or config/.
...
| --- | --- | --- |
| `config/` | KAI registry — bio, rank, preferences, authority tier | Persistent on disk. Source of truth for identity. |
...
- [ ]  Test Sanctuary Rule: attempt write to `config/` → verify hook blocks it
