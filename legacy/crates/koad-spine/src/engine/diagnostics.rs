...
pub struct ShipDiagnostics {
    ...
    curator: Arc<crate::engine::curator::CognitiveCurator>,
    config: Arc<koad_core::config::KoadConfig>,
...
impl ShipDiagnostics {
    ...
    pub async fn start_health_monitor(&self) {
        ...
        loop {
            ...
            let _ = self.curator.curate_intelligence().await;

            // 5.2 Cognitive Quicksave (Every 5 minutes)
            if iteration % 60 == 0 {
                let _ = self.curator.perform_cognitive_quicksave().await;
            }

            // 6. Long-cycle Pruning: Removed. Handled by integrated SessionReaper in ASM.
    ...
    async fn check_services(&self, registry: &mut HealthRegistry) -> anyhow::Result<()> {
        ...
        // 2. ASM Check (Now Integrated in Spine)
        let asm_status = "UP".to_string();

        registry.add(HealthCheck {
            name: "ASM Daemon".to_string(),
            status: HealthStatus::Pass,
            message: "Integrated ASM heartbeat active.".to_string(),
            last_checked: Utc::now().timestamp(),
            metadata: None,
        });
    ...
    async fn autonomic_recovery(&self, service: &str) -> anyhow::Result<()> {
        warn!("ShipDiagnostics: Health failure detected for {}. Reporting incident...", service);
        match service {
            "web-deck" => {
                let _ = self.report_incident(
                    "Sentinel:Gateway",
                    "ERROR",
                    "Gateway pulse lost. Manual or Watchdog recovery required.",
                    false,
                ).await;
            }
            "asm" => {
