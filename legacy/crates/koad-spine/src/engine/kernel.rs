...
impl Kernel {
    pub async fn shutdown(self) {
        info!("Kernel: Initiating graceful shutdown signal...");
        let _ = self.shutdown_tx.send(true);

        // Wait a moment for background tasks to notice
        tokio::time::sleep(std::time::Duration::from_millis(500)).await;
        info!("Kernel: Engine Room shutdown complete.");
    }
}
...
impl KernelBuilder {
    ...
    pub async fn start(self) -> anyhow::Result<Kernel> {
        ...
        let config_reaper = engine.asm.get_config();
        
        // ASM Monitor (Incoming messages)
        tokio::spawn(async move {
            tokio::select! {
                _ = asm.start_session_monitor() => {},
                _ = rx_asm.changed() => { info!("Kernel: ASM watcher stopping."); }
            }
        });

        // ASM Reaper (Periodic cleanup)
        let asm_reaper = engine.asm.clone();
        tokio::spawn(async move {
            tokio::select! {
                _ = asm_reaper.start_reaper(config_reaper) => {},
                _ = rx_reaper.changed() => { info!("Kernel: ASM reaper stopping."); }
            }
        });

        // Backup Manager (Periodic persistence)
        let backup = engine.backup.clone();
        let mut rx_backup = shutdown_rx.clone();
        tokio::spawn(async move {
            tokio::select! {
                _ = backup.start_backup_loop() => {},
                _ = rx_backup.changed() => { info!("Kernel: Backup manager stopping."); }
            }
        });

        let diag_inner = engine.diagnostics.clone();
        let rx_diag_task = shutdown_rx.clone();
        ...
        tokio::spawn(async move {
            loop {
                ...
                let handle = tokio::spawn(async move {
                    tokio::select! {
                        _ = diag_loop.start_health_monitor() => {
                            warn!("SHIP ALERT: Health monitor task exited unexpectedly.");
                        },
                        _ = rx_inner.changed() => {
                            info!("Kernel: Health monitor stopping.");
                        }
                    }
                });
                ...
                tokio::select! {
                    _ = handle => {
                        info!("Kernel: Diagnostic handle closed. Restarting in 5s...");
                    },
                    _ = diag_kill_rx.recv() => {
                        warn!("Kernel: Forcing restart of stalled diagnostic loop...");
                    }
                }
        ...
        tokio::spawn(async move {
            info!("Kernel: Autonomic Watchdog ACTIVE.");
            loop {
                ...
                if gap > 30 {
                    error!("CRITICAL: ShipDiagnostics loop stalled (Gap: {}s). Triggering task reset...", gap);
        ...
        tokio::spawn(async move {
            tokio::select! {
                ...
                _ = rx_router.changed() => { info!("Kernel: Directive router stopping."); }
            }
        });
        ...
        if let (Some(tcp_addr_str), Some(uds_path)) = (self.tcp_addr, self.uds_path) {
            ...
            tokio::spawn(async move {
                info!("Kernel: Launching Bridge gRPC (TCP) on {}...", tcp_addr);
        ...
        info!("Kernel: Engine Room stable.");
        info!("{}", engine.diagnostics.get_morning_report());
