...
mod tests {
    ...
    fn make_id(name: &str, rank: Rank) -> Identity {
        Identity {
            name: name.to_string(),
            rank,
            permissions: vec![],
    ...
    fn test_admin_bypass() {
        let id = make_id("dood", Rank::Admiral);
        assert!(matches!(
            Sandbox::evaluate(&id, "sudo rm -rf /"),
            PolicyResult::Allowed
        ));
    }
    ...
    fn test_developer_blacklist() {
        let dev = make_id("dev", Rank::Officer);
        assert!(matches!(
            Sandbox::evaluate(&dev, "sudo apt update"),
            PolicyResult::Denied(_)
        ));
        
        let pm = make_id("pm", Rank::Officer);
        assert!(matches!(
    ...
    fn test_developer_sanctuary() {
        let dev = make_id("dev", Rank::Officer);
        assert!(matches!(
            Sandbox::evaluate(&dev, "rm -rf .koad-os/koad.db"),
            PolicyResult::Denied(_)
        ));
        assert!(matches!(
            Sandbox::evaluate(&dev, "ls .koad-os"),
            PolicyResult::Allowed
        )); // Read is okay
    }
    ...
    fn test_developer_allowed() {
        let dev = make_id("dev", Rank::Officer);
        assert!(matches!(
            Sandbox::evaluate(&dev, "cargo build"),
            PolicyResult::Allowed
        ));
    }
    ...
    fn test_compliance_policy() {
        let comp = make_id("compliance", Rank::Officer);
        assert!(matches!(
            Sandbox::evaluate(&comp, "koad board status"),
            PolicyResult::Allowed
        ));
        
        let overseer = make_id("overseer", Rank::Officer);
        assert!(matches!(
