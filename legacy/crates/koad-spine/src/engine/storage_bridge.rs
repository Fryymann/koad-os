...
use chrono::Utc;
use fred::interfaces::{HashesInterface, KeysInterface, ClientLike};
...
impl KoadStorageBridge {
    ...
    pub async fn get_identity(&self, name: &str) -> anyhow::Result<Option<(String, String, String, i32)>> {
        let sqlite = self.sqlite.clone();
        let name_str = name.to_string();
        tokio::task::spawn_blocking(move || {
            let conn = sqlite.blocking_lock();
            let mut stmt = conn.prepare("SELECT name, role, bio, rank_tier FROM identities WHERE name = ?1")?;
            let mut rows = stmt.query(params![name_str])?;
            if let Some(row) = rows.next()? {
                Ok::<Option<(String, String, String, i32)>, anyhow::Error>(Some((
                    row.get(0)?,
                    row.get(1)?,
                    row.get(2)?,
                    row.get(3)?,
                ...
                Ok::<Option<(String, String, String, i32)>, anyhow::Error>(None)
            }
        })
        .await?
    }
    ...
    pub async fn get_identity_bio(&self, name: &str) -> anyhow::Result<Option<String>> {
        ...
        tokio::task::spawn_blocking(move || {
            let conn = sqlite.blocking_lock();
            let mut stmt = conn.prepare("SELECT bio FROM identities WHERE name = ?1")?;
            let mut rows = stmt.query(params![name_str])?;
            if let Some(row) = rows.next()? {
