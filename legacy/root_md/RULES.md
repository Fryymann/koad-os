...
1. **One Body, One Ghost**: A single CLI session (the Body) may host exactly one KAI Officer (the Ghost) at a time. The Body is identified by a unique `KOAD_BODY_ID`.
2. **The Tethering Rule**: A KAI is physically tethered to its session via the `KOAD_SESSION_ID` environment variable. This ID is the KAI's "lifeforce."
3. **Consciousness Collision**: Any attempt to boot a KAI in a session that already has an active `KOAD_SESSION_ID` MUST be rejected. This prevents nested consciousness and context pollution.
4. **Sovereign Override**: A **Sovereign** agent (Admiral or Captain) may use the `--force` flag to take over a Body if it is occupied by a stale or orphaned session. This action publishes a `SIG_FORCE_TAKEOVER` notification to the neural bus before the old Ghost is de-materialized.
5. **Officers vs. Drones**:
...
6. **The Vitality Pulse (Integrated Reaper)**: The KoadOS Spine (Kernel) is the authoritative reaper of stale sessions. If a KAI's heartbeat stops, the Spine shall mark it as "DARK" within `dark_timeout_secs` and purge it from hot state within the `purge_timeout_secs` threshold.
...
## **VIII. Cognitive Integrity & Hierarchy**

1. **Rank Hierarchy**: The KoadOS chain of command is: `Admiral` (Dood) > `Captain` (Tyr) > `Officer` (Sky, Vigil) > `Crew`.
2. **Tier 1 Requirement**: Admiral, Captain, and Officer ranks are **Sovereign-Class** cognitive identities. They MUST be hosted on **Tier 1** cognitive engines (Gemini 1.5 Pro/Ultra, Claude 3.5 Sonnet/Opus, or Codex) to ensure system stability and strategic reasoning.
3. **Cognitive Integrity Protocol (CIP)**: Agents with model tier > 1 are restricted from modifying "Sovereign State" keys in Redis (`identities`, `principles`, `canon_rules`).
4. **Cognitive Isolation**: Each agent's working memory is partitioned by `source_agent` in the Durable Memory Bank. Context snapshots are limited to the last 2 per agent to prevent history bloat.

## **IX. Configuration & Environment**

1. **TOML Hierarchy**: System configuration follows a strict override path: `kernel.toml` (Global) -> `registry.toml` (Project) -> `identities/*.toml` (Agent) -> `KOAD_` Environment Variables (Runtime).
2. **Authorization by Reference**: Agent `access_keys` in identity TOML files MUST store environment variable **names**, never literal secret values.
3. **Registry Discipline**: All path-to-repository mappings MUST be declared in `registry.toml`. Agents are forbidden from deriving GitHub contexts through heuristics if a registry entry exists.

---
*Status: CONDITION GREEN. Baseline Established 2026-03-03. Credential Sovereignty Added 2026-03-09. Architecture Refined 2026-03-11.*
