...
## Architectural Shift
1. **Unification:** Merge `KoadConfig` (currently environment variables) and `KoadLegacyConfig` (currently `config/` TOML files) into a single `koad_core::config::Config` struct using the `serde` and `figment` or `config` crates.
