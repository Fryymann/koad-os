...
- **Operational Files**: `config/`, `koad.db`, and `SESSION_LOG.md` are kept in the root but git-ignored.
...
2. **`install.sh` / `install.py`**:
   ...
   - **Actions**:
     ...
     - Generates the initial `config/` TOML files based on the agent's arguments.
