...
4. **Path Alignment**: Resolve everything through `filesystem.mappings` in `config/filesystem.toml`. Prefer `/home/ideans/data` so this driver shares the same workspace view as other agents.
5. **Tool Set**: Codex sessions only receive the shell-focused tools listed in `config/interfaces/codex.toml` (`run_shell_command`, `read_file`, `write_file`). Avoid calling external MCP-based tools unless explicitly allowed.
