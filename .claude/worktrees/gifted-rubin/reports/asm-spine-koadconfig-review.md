...
### 1.1 Boot Sequence
- **Resolution:** Agent name is currently resolved in `koad-cli/src/handlers/boot.rs` via `KoadLegacyConfig` (loading `koad.json`). My recent changes began transitioning this to the `KoadConfig` Registry (`crates/koad-core/src/config.rs`).
...
2. **High:** Identity Resolution (Full migration from `koad.json` to `config/identities/`).
