...
Minions are implemented as Claude Code native subagents — Markdown files with YAML frontmatter stored at `~/.claude/agents/clyde-minion.md` (user scope, available across all projects). The frontmatter defines the subagent; the body IS the boot contract.
...
```
~/.claude/agents/clyde-minion.md   ← primary definition (Claude Code)
...
Counters are stored at `.agents/clyde/bank/minion-counter.txt`:
```
I:0
C:0
```
...
Active and recent minion state is logged at `.agents/clyde/bank/minion-registry.md`:
...
- Write to `.agents/clyde/bank/minion-reports/{minion_id}.md` (report deposit only)
...
2. Write a `BLOCKED` report to `.agents/clyde/bank/minion-reports/{minion_id}.md`.
