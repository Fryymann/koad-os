...
- **System prompt:** Clyde's `.claude/agents/clyde.md` definition includes KoadOS identity, canon references, sovereignty rules, and the team lead behavioral contract. This is the KoadOS layer that native teams doesn't provide.
...
**Implementation:** Define as a reusable template in `.claude/agents/clyde-teammate.md` with YAML frontmatter. Clyde references this when spawning teammates.
...
| Minion boot contract (custom) | `clyde-teammate.md` agent definition | Author the `.claude/agents/clyde-teammate.md` file per spec in 5.1. |
