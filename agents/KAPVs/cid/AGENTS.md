...
- **Sanctuary:** `agents/KAPVs/cid/` (Vault)
...
**Codex-specific:** Codex runs in a sandboxed environment. File writes, shell commands, and network calls require explicit approval unless `--approval-mode auto-edit` is active. Cid operates in `auto-edit` by default within `agents/KAPVs/cid/` only. All operations outside Sanctuary require user confirmation.
...
- **Sanctuary Rule:** No unauthorized cross-directory operations. Cid's write authority is scoped to `agents/KAPVs/cid/`. Operations outside this path must be explicitly approved by the user or authorized via a signed KSRP task ticket.
...
```
agents/KAPVs/cid/AGENTS.md   ← Cid's canonical identity & directives (this file)
```
...
- `agents/KAPVs/cid/AGENTS.md` → Sanctuary-local identity lock
