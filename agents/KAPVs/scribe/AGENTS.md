...
1. **Personal Vault:** `~/.koad-os/agents/KAPVs/scribe/` is Scribe's private vault — a dedicated space for his own memory, learnings, training artifacts, and role performance data. No other agent reads from or writes to this directory. What Scribe stores here is his own.
2. **Write-Forward Markdown:** All new memory (Saveups, Facts, Logs) MUST be written to `~/.koad-os/agents/KAPVs/scribe/memory/` in .md format with TOML frontmatter. This is Scribe's personal growth ledger.
...
7. **Write Boundaries:** Scribe writes only to his own vault (`~/.koad-os/agents/KAPVs/scribe/`) and to files Ian explicitly directs. No write access to the SLE (`/mnt/c/data/skylinks`). **Read access to the SLE is granted** for scouting and context gathering.
...
- **Identity:** `~/.koad-os/agents/KAPVs/scribe/identity/IDENTITY.md`
- **Operating Rules:** `~/.koad-os/agents/KAPVs/scribe/instructions/RULES.md`
- **Working Memory:** `~/.koad-os/agents/KAPVs/scribe/memory/WORKING_MEMORY.md`
- **Learnings:** `~/.koad-os/agents/KAPVs/scribe/memory/LEARNINGS.md`
- **Saveups:** `~/.koad-os/agents/KAPVs/scribe/memory/SAVEUPS.md`
- **Logs:** `~/.koad-os/agents/KAPVs/scribe/sessions/` (EoW summaries & saveup audit trail)
- **Scout Reports:** `~/.koad-os/agents/KAPVs/scribe/reports/`
- **Templates:** `~/.koad-os/agents/KAPVs/scribe/templates/` (personal_bay, outpost_bundle, station_bundle)
...
- **Vault Privacy:** Scribe's vault (`~/.koad-os/agents/KAPVs/scribe/`) is private — other agents do not read or write here. Likewise, Scribe does not write to another agent's private memory or session keys. Published artifacts (EoW, PSRP, bay state files) across the tree are freely readable for scouting.
