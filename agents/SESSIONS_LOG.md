---
## Clyde — Session 9 — 2026-03-25

**Work Performed:** gRPC boot timeout fix + CASS EndOfWatch spam fix

### Task 1: koad-agent.rs gRPC Boot Timeout

Implemented the pre-approved plan (`validated-jumping-charm.md`). Both gRPC call blocks in `crates/koad-cli/src/bin/koad-agent.rs` replaced with Tonic `Endpoint` builder:
...
`BOOT_SERVICE_TIMEOUT = Duration::from_secs(3)`. Added `use std::time::Duration` and `use tonic::transport::Endpoint` imports. Committed `380a4b2` on `worktree-clyde-grpc-timeout`, merged to `nightly` at `06c971d`. Worktree removed. Build clean. Live mode confirmed <2s. Dark mode test (requires sudo stop) pending user validation.

### Task 2: CASS EndOfWatch XREADGROUP spam

**Symptom:** `EndOfWatch: Stream read error: XREADGROUP failed` every ~10s in CASS logs.

**Root cause:** fred 9.x `xreadgroup_map` calls `HashMap::from_value(Null)` when `XREADGROUP BLOCK 5000` times out (no messages). This calls `into_map()` on `RedisValue::Null`. The `Null` arm in `into_map()` is gated behind `#[cfg(feature = "default-nil-types")]` — without it, nil hits the `_` error arm.

**Fix:** Added `"default-nil-types"` to fred workspace features in `Cargo.toml`. Rebuilt `koad-cass`. Installed new binary. Both error logs (`cass.error.log`, `citadel.error.log`) are now static — verified no growth over 15s with two successive `wc -l` checks.

### Jupiter State Post-Session
- Citadel (:50051): ACTIVE
- CASS (:50052): ACTIVE — EoW spam resolved
- Redis: PONG
- agent-boot: ~1s (dark-mode cap: ~6s)
- Qdrant: still OFFLINE
