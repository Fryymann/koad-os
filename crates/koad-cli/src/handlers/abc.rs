...
impl AbcCollector {
    ...
    pub async fn collect_raw_data(&self) -> Result<String> {
        ...
        raw.push_str(&gh_items);
        raw.push('\n');

        // 4. Active CASS Pulses (Global Awareness)
        raw.push_str("--- ACTIVE CASS PULSES ---\n");
        match koad_proto::cass::v1::pulse_service_client::PulseServiceClient::connect(self.config.network.cass_grpc_addr.clone()).await {
            Ok(mut client) => {
                let req = koad_proto::cass::v1::GetPulsesRequest {
                    context: None,
                    role: "global".to_string(),
                };
                if let Ok(resp) = client.get_pulses(req).await {
            ...
            Err(_) => raw.push_str("_CASS offline — pulses unavailable._\n"),
        }
