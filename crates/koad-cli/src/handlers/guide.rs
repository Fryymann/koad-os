...
fn show_quick_start() {
    ...
    println!("2. \x1b[1mOrient:\x1b[0m Read your Context Packet (CASS) and `koad map look`.");
    println!("3. \x1b[1mResearch:\x1b[0m Use `grep_search` and API maps. Never read full files > 50 lines.");
    println!("4. \x1b[1mStrategy:\x1b[0m Formulate a plan. Use `enter_plan_mode` for Medium+ tasks.");
    println!("5. \x1b[1mExecution:\x1b[0m Surgical updates only. Follow the Research -> Strategy -> Execution cycle.");
    println!("6. \x1b[1mValidate:\x1b[0m Run tests and linters. No change is complete without verification.");
...
fn show_prime_directives() {
    ...
    println!("4. \x1b[1mNo-Read Rule:\x1b[0m Forbidden from reading entire files over 50 lines. Use surgical extraction.");
    println!("5. \x1b[1mDood Approval:\x1b[0m Major architectural changes require human (Ian) oversight.");
    println!("6. \x1b[1mSecure Cognition:\x1b[0m Zero tolerance for secret leakage. Use the Vault.");
}

fn show_standard_workflow() {
    println!("\x1b[1;32m--- Standard Engineering Cycle ---\x1b[0m");
    println!("\x1b[1m[1] Research:\x1b[0m Map codebase, validate assumptions, reproduce bugs.");
    println!("\x1b[1m[2] Strategy:\x1b[0m Draft a design/plan. Share summary for approval.");
    println!("\x1b[1m[3] Execution:\x1b[0m Surgical Plan -> Act -> Validate loop per sub-task.");
    println!("\x1b[1m[4] Verification:\x1b[0m Exhaustive tests, linting, and type-checking.");
    println!("\x1b[1m[5] Documentation:\x1b[0m Update logs (SESSIONS_LOG, WORKING_MEMORY, Updates).");
}
