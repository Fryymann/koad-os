...
pub async fn handle_boot_command(
    ...
    if let Ok(existing_sid) = env::var("KOAD_SESSION_ID") {
        if !existing_sid.is_empty() {
            ...
            let val: Option<String> = rc.pool.hget("koad:state", &session_key).await.unwrap_or(None);
    ...
    let hydration_resp = cass.hydrate(HydrationRequest {
    ...
    if compact {
        println!(
            ...
            agent, role, session_id, session_id, body_id, context_file.display()
        );
    } else {
        println!("\n\x1b[1m--- KoadOS Neural Link Established ---\x1b[0m");
        println!("Agent:    {}", agent);
        println!("Session:  {}", session_id);
        println!("Context:  {} ({} tokens)", context_file.display(), packet.estimated_tokens);
...
pub async fn handle_logout_command(session: Option<String>, config: &KoadConfig) -> Result<()> {
    ...
    client.close_session(CloseRequest {
        context: None,
