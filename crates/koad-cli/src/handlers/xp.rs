...
pub async fn handle_xp_command(command: XpCommands, config: &KoadConfig) -> Result<()> {
    ...
    match command {
        XpCommands::Status { agent } => {
            ...
            println!("\x1b[1mTier:\x1b[0m       \x1b[32m{}\x1b[0m (Level {})", resp.tier_name, resp.level);
            ...
            println!("\x1b[1mProgress:\x1b[0m   {}  {:.0}%", bar_str, progress * 100.0);
            println!("\x1b[1mNext Level:\x1b[0m {} XP", next_total);
            println!("");
        }
        XpCommands::Award { agent, amount, reason, source } => {
