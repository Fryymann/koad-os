...
pub async fn show_motd(agent_name: &str, config: &KoadConfig) -> Result<()> {
    ...
    // 1. Fetch XP Stats
    let xp_client = XpServiceClient::connect(config.network.citadel_grpc_addr.clone()).await.ok();
    let xp_status = if let Some(mut client) = xp_client {
        client.get_status(XpStatusRequest {
            context: context.clone(),
            agent_name: agent_name.to_string(),
        }).await.ok().map(|r| r.into_inner())
    } else {
        None
    };

    // 2. Fetch Pending Signals (Messages/Notes)
    let signal_client = SignalClient::connect(config.network.citadel_grpc_addr.clone()).await.ok();
    let pending_signals = if let Some(mut client) = signal_client {
        ...
        }).await.ok().map(|r| r.into_inner().signals)
    } else {
        None
    };
    ...
    if let Some(id) = identity {
        ...
        println!("  \x1b[1mAgent:\x1b[0m      {} (\x1b[32m{}\x1b[0m)", id.name, id.rank);
    ...
    if let Some(xp) = xp_status {
        ...
        let bar_str = format!("\x1b[32m{}\x1b[0m{}", "█".repeat(bars), "░".repeat(20 - bars));
        
        println!("  \x1b[1mTier:\x1b[0m       \x1b[32m{}\x1b[0m (Level {})", xp.tier_name, xp.level);
        println!("  \x1b[1mProgress:\x1b[0m   {}  {:.1}%", bar_str, progress * 100.0);
    ...
    if let Some(signals) = pending_signals {
        if signals.is_empty() {
            ...
            for (i, sig) in signals.iter().take(3).enumerate() {
                let prefix = if i == 2 && signals.len() > 3 { "  └─ ..." } else { "  ├─" };
                println!("  {} [{}] from {}: {}", prefix, &sig.id[..4], sig.source_agent, 
                    if sig.message.len() > 50 { format!("{}...", &sig.message[..47]) } else { sig.message.clone() }
                );
            }
        }
    } else {
