...
pub async fn handle_cognitive_check(
    ...
    println!("
[1m--- {} Cognitive Self-Health Report ---[0m", agent_name);
    ...
    let signals: std::collections::HashMap<String, String> = client.pool.hgetall(&mailbox_key).await.unwrap_or_default();
    let pending_signals = signals.values().filter(|v| v.contains("pending")).count();

    println!("[32m[PASS][0m L2: Hot Context ({} chunks active)", chunks.len());
    println!("[32m[PASS][0m L2: Mailbox ({} pending signals)", pending_signals);
    ...
    match db.get_conn() {
        Ok(conn) => {
            ...
            println!("[32m[PASS][0m L3: Deep Memory ({} records ingested)", count);
        }
    ...
    let is_heartbeat_running = sys.processes().values().any(|p| p.name().contains("koad") && p.cmd().iter().any(|c| c == "heartbeat"));
    ...
    if report_dir.exists() {
        ...
        println!("[32m[PASS][0m L5: Procedural ({} reports accessible)", entries);
    } else {
