        ...
        if is_spine || is_gateway {
            ...
            if let Some(ph) = process_home {
                ...
                if let Some(exe_path) = process.exe() {
                    let real_exe =
                        std::fs::read_link(exe_path).unwrap_or_else(|_| exe_path.to_path_buf());

                    if let (Ok(exe_can), Ok(home_bin_can)) =
                        (real_exe.canonicalize(), home.join("bin").canonicalize())
                    {
                        if exe_can.starts_with(&home_bin_can) {
                                ...
                                if let Ok(modified) = metadata.modified() {
                                    ...
                                    if modified > start_time {
                                        ghosts.push((
                                            pid_u32,
                                            format!(
                                                "STALE {}: Binary updated since process started.",
                                                name
                                            ),
                                        ));
                                    }
                            ...
                            ghosts.push((
                                pid_u32,
                                format!("Ghost {} (Foreign Binary: {})", name, exe_can.display()),
                            ));
                        }
                    }
                }
...
impl KoadDB {
    ...
    fn verify_role(&self, identity_id: &str, role: &str) -> Result<bool> {
        let conn = self.get_conn()?;
        let mut stmt =
            conn.prepare("SELECT 1 FROM identity_roles WHERE identity_id = ?1 AND role = ?2")?;
        let exists = stmt
            .query(params![identity_id, role.to_lowercase()])?
            .next()?
            .is_some();
        Ok(exists)
    }
    ...
    fn remember(&self, cat: &str, content: &str, tags: Option<String>, tier: i32) -> Result<()> {
        if tier > 1 {
            anyhow::bail!(
                "Cognitive Protection: Model Tier {} is not authorized to write to Memory Bank.",
                tier
            );
        }
        let conn = self.get_conn()?;
        ...
        conn.execute(
            ...
            params![cat, content, tags.unwrap_or_default(), ts],
        )?;
        Ok(())
    }
    ...
    fn get_contextual(&self, limit: usize, tags: Vec<String>) -> Result<Vec<(String, String)>> {
        ...
        for t in tags {
            ...
            let rows = stmt.query_map(params![format!("%{}%", t), limit], |row| {
    ...
    > {
        let conn = self.get_conn()?;
        let mut stmt = conn
            .prepare("SELECT name, path, branch, health, last_sync FROM projects WHERE id = ?1")?;
        let mut rows = stmt.query(params![id])?;
        if let Some(row) = rows.next()? {
            Ok(Some((
                row.get(0)?,
    ...
    fn update_project_status(
        ...
        match (branch, health) {
            (Some(b), Some(h)) => {
                conn.execute(
                    "UPDATE projects SET branch = ?1, health = ?2, last_sync = ?3 WHERE id = ?4",
                    params![b, h, ts, id],
                )?;
            }
            (Some(b), None) => {
                conn.execute(
                    "UPDATE projects SET branch = ?1, last_sync = ?2 WHERE id = ?3",
                    params![b, ts, id],
                )?;
            }
            (None, Some(h)) => {
                conn.execute(
                    "UPDATE projects SET health = ?1, last_sync = ?2 WHERE id = ?3",
                    params![h, ts, id],
                )?;
            }
            _ => {}
    ...
    fn get_spec(&self) -> Result<Option<(String, String, String, String)>> {
        ...
        let mut stmt = conn.prepare(
            "SELECT content, timestamp, 'active', 'active' FROM active_spec WHERE active=1 LIMIT 1",
        )?;
    ...
    fn get_notes(&self, limit: usize) -> Result<Vec<(i32, String, String)>> {
        let conn = self.get_conn()?;
        let mut stmt =
            conn.prepare("SELECT id, content, timestamp FROM notes ORDER BY id DESC LIMIT ?1")?;
    ...
    fn get_recent_brainstorms(&self, limit: usize) -> Result<Vec<(String, String, String)>> {
        let conn = self.get_conn()?;
        let mut stmt = conn.prepare(
            "SELECT content, category, timestamp FROM brainstorms ORDER BY id DESC LIMIT ?1",
        )?;
        let rows = stmt.query_map([limit], |row| Ok((row.get(0)?, row.get(1)?, row.get(2)?)))?;
        let mut results = Vec::new();
        for r in rows {
...
fn get_gh_pat_for_path(path: &Path, role: &str, _config: &KoadConfig) -> (String, String) {
    if role.to_lowercase() == "admin" {
        return ("GITHUB_ADMIN_PAT".into(), "Admin".into());
    }
    let path_str = path.to_string_lossy();
    if path_str.contains("skylinks") {
        ("GITHUB_SKYLINKS_PAT".into(), "Skylinks".into())
    } else {
...
fn get_gdrive_token_for_path(path: &Path) -> (String, String) {
    let path_str = path.to_string_lossy();
    if path_str.contains("skylinks") {
        ("GDRIVE_SKYLINKS_TOKEN".into(), "Skylinks".into())
    } else {
        ("GDRIVE_PERSONAL_TOKEN".into(), "Personal".into())
    }
}

fn feature_gate(feature: &str, issue_num: Option<u32>) {
    let issue_str = issue_num
        .map(|n| format!(" (See Issue #{})", n))
        .unwrap_or_default();
    println!(
...
async fn main() -> Result<()> {
    ...
    let legacy_config = KoadLegacyConfig::load(&config.home)
        .unwrap_or_else(|_| KoadLegacyConfig::default_initial());
    ...
    match cli.command {
        Commands::Boot {
            ...
            let (final_agent_name, final_role, final_bio) = if let Some(identity) =
                db.get_identity(&agent)?
            {
                if !db.verify_role(&agent, &role)? {
                    anyhow::bail!(
                        "Identity '{}' is not authorized for the '{}' role.",
                        agent,
                        role
                    );
                }
                ...
                warn!(
                    "Identity '{}' not found in registry. Defaulting to Guest (restricted).",
                    agent
                );
                (
                    agent.clone(),
                    "guest".to_string(),
                    "Unverified Agent".to_string(),
                )
            };
                            ...
                            .query::<Option<String>>(&mut con)
                        {
                            if let Ok(data) = serde_json::from_str::<Value>(&json_str) {
                                if let Some(briefing) = data.get("mission_briefing") {
                                    mission_briefing =
                                        Some(briefing.as_str().unwrap_or_default().to_string());
                                }
                            }
                        }
                    }
            ...
            if compact {
                ...
                println!(
                    "<koad_boot>\nSession:  {}\nIdentity: {} ({})\nTier:     {}",
                    session_id, final_agent_name, final_role, model_tier
                );
        ...
        Commands::Doctor => {
            ...
            if legacy_config.identity.name != "Koad" || legacy_config.identity.role != "Admin" {
                println!(
                    "\x1b[33m[WARN]\x1b[0m Non-standard persona detected: {}",
                    legacy_config.identity.name
                );
            } else {
                println!(
                    "\x1b[32m[PASS]\x1b[0m Persona: {} ({})",
                    legacy_config.identity.name, legacy_config.identity.role
                );
            }
                    ...
                    for line in String::from_utf8_lossy(&out.stdout).lines() {
                        if let Some(project_root) = PathBuf::from(line).parent() {
                            let name = project_root
                                ...
                                .to_string_lossy();
                            if db
                                .register_project(&name, &project_root.to_string_lossy())
                                .is_ok()
                            {
        ...
        Commands::Publish { message } => {
            ...
            let m = message.unwrap_or_else(|| {
                format!("KoadOS Sync - {}", Local::now().format("%Y-%m-%d %H:%M"))
            });
            Command::new("git")
                .arg("-C")
                .arg(&h)
        ...
        Commands::Saveup {
            ...
            db.remember(
                "fact",
                &format!("Saveup ({}): {} | Facts: {}", scope, summary, fact_str),
                Some(scope.clone()),
                model_tier,
            )?;
        ...
        Commands::Gcloud { .. } => {
            feature_gate("koad gcloud", None);
        }
        Commands::Airtable { .. } => {
            feature_gate("koad airtable", None);
        }
        Commands::Sync { .. } => {
            feature_gate("koad sync", None);
        }
        Commands::Drive { .. } => {
            feature_gate("koad drive", None);
        }
        Commands::Stream { .. } => {
            feature_gate("koad stream", None);
        }
        Commands::Skill { .. } => {
            feature_gate("koad skill", None);
        }
        Commands::Whoami => {
            println!(
                "Persona: {} ({})",
        ...
        Commands::Board { action } => {
            ...
            match action {
                BoardAction::Status => {
                    println!(
                        ">>> [UPLINK] Accessing Neural Log: Tactical Overlay (Project #{})...",
                        project_num
                    );
                    let items = client.list_project_items(project_num).await?;
                    println!(
                        "\n{:<5} {:<50} {:<15} {:<15}",
                        "NODE", "DATA FRAGMENT", "STATUS", "VERSION"
                    );
                    println!("{:-<90}", "");
                    for item in items {
                ...
                BoardAction::Sync => {
                    if !is_admin {
                        anyhow::bail!("Admin Auth Required for Board Sync.");
                    }
                    client.sync_issues(project_num).await?;
                }
                BoardAction::Done { id } => {
                    if !is_admin {
                        anyhow::bail!("Admin Auth Required to Close Nodes.");
                    }
                    client.update_item_status(project_num, id, "Done").await?;
                }
                BoardAction::Todo { id } => {
                    if !is_admin {
                        anyhow::bail!("Admin Auth Required to Reopen Nodes.");
                    }
                    client.update_item_status(project_num, id, "Todo").await?;
                }
                BoardAction::Verify { id } => {
                    println!(
                        ">>> [VERIFY] Cross-referencing Node #{} with Command Deck...",
                        id
                    );
                    let items = client.list_project_items(project_num).await?;
                    if let Some(item) = items.iter().find(|i| i.number == Some(id)) {
                        println!(
                            "  [PASS] Node #{} verified. Current Status: {}",
                            id, item.status
                        );
                    } else {
                        anyhow::bail!(
                            "Node #{} not found on Project Board. Manual sync required.",
                            id
                        );
                    }
                }
                BoardAction::Sdr => {
                    feature_gate("koad board sdr", None);
                }
            }
        }
        Commands::Mind { action } => {
            ...
            match action {
                MindAction::Status => {
                    ...
                    let learn_count: i32 = conn.query_row(
                        "SELECT count(*) FROM learnings WHERE status = 'active'",
                        [],
                        |r| r.get(0),
                    )?;
                    let decision_count: i32 =
        ...
        Commands::Project { action } => match action {
            ProjectAction::List => {
                ...
                for (id, name, path, branch, health) in projects {
                    ...
                    println!(
                        "{:<4} {:<20} {:<15} {}{:<10}\x1b[0m {}",
                        id, name, branch, health_color, health, path
                    );
                }
                println!();
            }
                ...
                let project_id = match id {
                    ...
                    None => {
                        ...
                        project_id.ok_or_else(|| {
                            anyhow::anyhow!("Not inside a registered project. Provide an ID.")
                        })?
                    }
                };
        ...
        Commands::Issue { action } => {
            ...
            match action {
                ...
                IssueAction::Move { number, step } => {
                    if step < 1 || step > 8 {
                        anyhow::bail!("Invalid step. Agents can only move through steps 1-8.");
                    }

                    // CRITICAL: Block agents from entering Step 5 (Implement) via 'move'
                    if step == 5 {
                        anyhow::bail!("Access Denied: Step 5 (Implement) requires explicit Admin Approval. Run 'koad issue approve' instead.");
                    }

                    let mut stmt =
                        conn.prepare("SELECT canon_step FROM task_graph WHERE issue_number = ?1")?;
                    let current_step: i32 = stmt
                        .query_row([number], |row| row.get(0))
                        .context("Issue not tracked. Run 'koad issue track' first.")?;
                    ...
                    println!(
                        "\x1b[34m[MOVE]\x1b[0m Node #{} advanced to Step {}.",
                        number, step
                    );
                }
                IssueAction::Approve { number } => {
                    if !is_admin {
                        anyhow::bail!("Access Denied: Only Admin can approve safety gates.");
                    }
                ...
                IssueAction::Close { number } => {
                    ...
                    let current_step: i32 = stmt
                        .query_row([number], |row| row.get(0))
                        .context("Issue not tracked.")?;

                    if current_step != 9 {
                        anyhow::bail!("LOCKED: Node #{} cannot be closed. Current Step: {}. (Required: 9 - Approved)", number, current_step);
                    }

                    println!(
                        ">>> [UPLINK] Authenticating closure for Node #{}...",
                        number
                    );
                    let token = config.resolve_gh_token()?;
                    let client = GitHubClient::new(token, "Fryymann".into(), "koad-os".into())?;
                    client
                        .update_item_status(config.github_project_number as i32, number, "Done")
                        .await?;
                    ...
                    println!(
                        "\x1b[32m[FINALIZED]\x1b[0m Node #{} is closed and archived.",
                        number
                    );
                }
                IssueAction::Status { number } => {
                    ...
                    let (step, desc, status): (i32, String, String) = stmt
                        .query_row([number], |row| Ok((row.get(0)?, row.get(1)?, row.get(2)?)))?;
                    println!(
                        "\n\x1b[1m--- Sovereignty Status: Node #{} ---\x1b[0m",
                        number
                    );
        ...
        Commands::Crew => {
            if !config.redis_socket.exists() {
                anyhow::bail!(
                    "Kernel offline (Redis UDS missing). Cannot fetch live crew manifest."
                );
            }
            ...
            let sessions: HashMap<String, String> =
                redis::cmd("HGETALL").arg("koad:state").query(&mut con)?;
            ...
            for (key, val) in sessions {
                if key.starts_with("koad:session:") {
                    if let Ok(data) = serde_json::from_str::<Value>(&val) {
                        ...
                        let status = if let Ok(last_hb) =
                            chrono::DateTime::parse_from_rfc3339(last_hb_str)
                        {
                            let diff =
                                Utc::now().signed_duration_since(last_hb.with_timezone(&Utc));
        ...
        Commands::Refresh { restart } => {
            ...
            if !build_status.success() {
                anyhow::bail!(
                    "Forge failure: Build failed with exit code {}",
            ...
            for (src, dest) in bins {
                ...
                if src_path.exists() {
                    ...
                    if let Err(e) = std::fs::copy(&src_path, &dest_path) {
                        ...
                        if old_path.exists() {
                            let _ = std::fs::rename(&old_path, &dest_path);
                        }
                    } else {
