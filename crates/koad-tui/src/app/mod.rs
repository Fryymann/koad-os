...
pub async fn run_tui() -> Result<()> {
    ...
    let mut telemetry_stream = client
    ...
    loop {
        ...
        if let Ok(Ok(Some(update))) =
            tokio::time::timeout(Duration::from_millis(1), telemetry_stream.message()).await
        {
        ...
        terminal.draw(|f| {
            ...
            if let Some(s) = &app.stats {
                state_items.push(
                    ListItem::new(format!("CPU: {:.1}%", s.cpu_usage))
                        .style(Style::default().fg(Color::Yellow)),
                );
                state_items.push(
                    ListItem::new(format!("MEM: {} MB", s.memory_usage))
                        .style(Style::default().fg(Color::Cyan)),
                );
                state_items.push(
                    ListItem::new(format!("UPTIME: {}s", s.uptime))
            ...
            let telemetry_items: Vec<ListItem> = app
                .terminal_logs
                .iter()
                .rev()
                .take(20)
