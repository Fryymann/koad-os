...
pub fn build_citadel_interceptor(sessions: ActiveSessions) -> impl Fn(Request<()>) -> Result<Request<()>, Status> + Clone {
    move |req: Request<()>| {
        ...
        // We use blocking_lock here as interceptors are called in a sync context by Tonic.
        // This is safe because HashMap lookups are O(1) and the mutex is rarely contested.
        let sessions_guard = sessions.blocking_lock();
