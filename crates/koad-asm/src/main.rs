...
impl SessionManager {
    ...
    async fn run(&self) -> Result<()> {
        ...
        while let Ok(message) = message_stream.recv().await {
            ...
            info!("ASM: Received raw bus message on {}: {}", message.channel, payload_str);
                ...
                match msg_type.as_str() {
                    "SESSION_UPDATE" => {
                        if let Ok(session) = serde_json::from_value::<AgentSession>(msg["data"].clone()) {
                            info!("ASM: Session Update -> KAI: '{}' (ID: {}, Status: {})", 
    ...
    async fn static_prune(pool: &RedisPool) -> Result<()> {
            ...
            if key.starts_with("koad:session:") {
                if let Ok(raw_json) = serde_json::from_str::<serde_json::Value>(&val) {
                    ...
                    if let Ok(session) = serde_json::from_value::<AgentSession>(data.clone()) {
                        if session.identity.name == "Koad" || session.identity.name == "Tyr" || session.identity.name == "Dood" {
                            continue;
                        }
        ...
        for (key, session) in to_dark {
            ...
            let _: () = pool.next().hset("koad:state", (&key, payload.to_string())).await?;
